...
RSpec.describe 'ContestPolicy', :ui do
  describe 'registration' do
    ...
    context 'when open' do
      ...
      it { expect(page).to have_content 'РЕЄСТРАЦІЯ УЧАСНИКА' }
      it { expect(page).to_not have_content 'Реєстрація заборонена. Повідомте технічного працівника.' }
  ...
  describe 'viewing content' do
    ...
    context 'when task closed' do
      ...
      it { expect(page).to_not have_content 'Додаткові матеріали' }
    end

    context 'when task open' do
      let(:contest) { create :contest, task_open: true }

      it { expect(page).to_not have_content 'Перегляд завдання заборонено. Повідомте технічного працівника.' }
  ...
  describe 'upload' do
    ...
    context 'when open' do
      ...
      it { expect(page).to have_content 'ВІДПРАВКА РОЗВ\'ЯЗКІВ' }
      it { expect(page).to_not have_content 'Відправка розв\'язків заборонена. Повідомте технічного працівника.' }
