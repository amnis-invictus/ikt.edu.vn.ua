$(document).ready(function () {
  ...
  $(document).on('submit', 'form[data-coh]', function (e) {
    ...
    if ($form.attr('data-direct-uploads-processing') !== undefined) {
      logger.info('COH integration: direct uploads in progress, returning');
      return;
    }

    if ($form.find('input[type=file][data-direct-upload-url]:not([disabled])').length > 0) {
      logger.info('COH integration: direct uploads pending, returning');
      return;
    }
